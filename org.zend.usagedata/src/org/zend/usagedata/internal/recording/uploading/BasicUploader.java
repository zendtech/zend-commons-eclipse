...
public class BasicUploader extends AbstractUploader {
		...
		try {
			...
			if (result.isSuccess()) {
				UsageDataActivator
						.getDefault()
						.log(IStatus.INFO,
								"Usage data uploaded to %1$s in %2$s milliseconds.", getUploadUrl(), duration); //$NON-NLS-1$
			} else {
				UsageDataActivator
						.getDefault()
						.log(IStatus.INFO,
								"Usage data upload to %1$s failed with error code %2$s.", getUploadUrl(), result.getReturnCode()); //$NON-NLS-1$
			}
			...
			UsageDataActivator
					...
					.log(IStatus.WARNING,
							e,
							"The URL provided for usage data upload, %1$s, is invalid.", getUploadUrl()); //$NON-NLS-1$
		} catch (UnknownHostException e) {
			UsageDataActivator
					.getDefault()
					.log(IStatus.WARNING,
							e,
							"The usage data upload server at %1$s could not be found.", getUploadUrl()); //$NON-NLS-1$
		} catch (ConnectException e) {
			UsageDataActivator
					.getDefault()
					.log(IStatus.WARNING,
							e,
							"Could not connect to the usage data upload server at %1$s.", getUploadUrl()); //$NON-NLS-1$
		} catch (InterruptedIOException e) {
			UsageDataActivator
					.getDefault()
					.log(IStatus.WARNING, e,
							"A socket timeout occurred while trying to upload usage data."); //$NON-NLS-1$
		} catch (Exception e) {
			UsageDataActivator.getDefault().log(IStatus.WARNING, e,
					"An exception occurred while trying to upload usage data."); //$NON-NLS-1$
		}
	...
	void handleServerResponse(PostMethod post) {
		...
		try {
			...
			UsageDataActivator.getDefault().log(IStatus.WARNING, e,
					"Exception raised while parsing the server response"); //$NON-NLS-1$
		} finally {
			try {
				response.close();
			} catch (IOException e) {
	...
	void handleServerResponse(BufferedReader response) throws IOException {
		while (true) {
			...
			if (getSettings().isLoggingServerActivity()) {
				UsageDataActivator.getDefault().log(IStatus.INFO, line);
			}
