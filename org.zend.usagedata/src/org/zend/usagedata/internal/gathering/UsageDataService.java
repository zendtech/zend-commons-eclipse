...
public class UsageDataService implements IUsageDataService {
	private static final String MONITORS_EXTENSION_POINT = UsageDataActivator.PLUGIN_ID
			+ ".monitors"; //$NON-NLS-1$

	/**
	 * A temporary home for events as they are generated. As they are created,
	 * events are dropped into the queue by the source thread. Events are
	 * consumed from the queue by the {@link #eventConsumerJob}.
	 * 
	 ...
	 * {@link IUsageMonitor}. Strictly speaking this is not a list of
	...
	/**
	 * The list of objects of type {@link UsageDataEventListener} listening to
	 ...
	 * in anticipation of performance issues (see
	 * {@link #startEventConsumerJob()} for discussion.
	 */
	private Job eventConsumerJob;

	/**
	 * This field maps the symbolic name of bundles to the last loaded version.
	 * This information is handy for filling in missing bundle version
	 * information for singleton bundles.
	 * 
	 ...
	 * 
	 * @see
	 * org.zend.usagedata.internal.gathering.IUsageDataService#startMonitoring()
	 */
	 ...
	 * 
	 * @see
	 * org.zend.usagedata.internal.gathering.IUsageDataService#stopMonitoring()
	 */
	 ...
	 * 
	 * @see
	 * org.zend.usagedata.internal.gathering.IUsageDataService#isMonitoring()
	 */
	 ...
	 * 
	 * @see
	 * org.zend.usagedata.internal.gathering.IUsageDataService#recordEvent(java
	...
	public void recordEvent(String what, String kind, String description,
		...
		UsageDataEvent event = new UsageDataEvent(what, kind, description,
				bundleId, bundleVersion, System.currentTimeMillis());
		recordEvent(event);
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.zend.usagedata.internal.gathering.IUsageDataService#
	 * addUsageDataEventListener
	 * (org.zend.usagedata.internal.gathering.UsageDataEventListener)
	 */
	 ...
	 * dispatches them to the various {@link UsageDataEventListener}s. Since the
	 * event listeners will do expensive things like open and write to files, it
	 * is anticipated that this architecture will allow the necessary activities
	 * to happen without significantly impacting the user's experience.
	 */
	protected void startEventConsumerJob() {
		...
		eventConsumerJob.schedule(1000); // Wait a few minutes before scheduling
											// the job.
	}

	/**
	 * This method pauses the current thread until the workbench has finished
	 * starting. This should provide enough time for bundles that are installing
	 * usage data event listeners to complete before events are dispatched.
	 */
	protected void waitForWorkbenchToFinishStarting() {
		/*
		 * We want the job to pause until after all the bundles that are loaded
		 * at startup have finished loading. This will give bundles that listen
		 * to usage data events time to load and install listeners before events
		 * are fired off (which should mean that events won't get lost).
		 * 
		 * I had originally tried using Display.syncExec(Runnable) (with an
		 * "do nothing" Runnable, but this caused some weird classloading issues
		 * similar to those referenced in Bug 88109.
		 */
	...
	protected void startMonitors() {
		IConfigurationElement[] elements = Platform.getExtensionRegistry()
				.getConfigurationElementsFor(MONITORS_EXTENSION_POINT);
		for (IConfigurationElement element : elements) {
			if ("monitor".equals(element.getName())) { //$NON-NLS-1$

				try {
					Object monitor = element.createExecutableExtension("class"); //$NON-NLS-1$
					if (monitor instanceof IUsageMonitor) {
						startMonitor((IUsageMonitor) monitor);
					}
	...
	protected void stopMonitors() {
		for (Object monitor : monitors.getListeners()) {
			stopMonitor((IUsageMonitor) monitor);
		}
	}
	...
	/**
	 * This method does the actual dispatching of the event to a single
	 * listener. If an exception occurs in the execution of the listener, an
	 * exception is logged.
	 * 
	 * @param event
	 * @param listener
	 */
	private void dispatchEvent(UsageDataEvent event,
			UsageDataEventListener listener) {
		try {
			...
			UsageDataActivator.getDefault().log(
					IStatus.ERROR,
					e,
					"The listener (" + listener.getClass()
							+ ") threw an exception");
		}
	}
	...
	private void registerBundleVersion(UsageDataEvent event) {
		 ...
		 * from--though this knowledge does constitute a relatively tight form
		 * of coupling). If the event tells us that a bundle has been started,
		 * we'll move on; otherwise, we bail out. We're not interested in other
		 * bundle events (like stops, etc.), since these events will be
		 * relatively rare for the kinds of bundles we actually care about.
		 */
	...
	private void startMonitor(IUsageMonitor monitor) {
		monitor.startMonitoring(this);
		monitors.add(monitor);
	}

	private void stopMonitor(IUsageMonitor monitor) {
